...
void list_free_callback(t_list *list, ffree free);
...
#define lf_1(list) t_list *__list = (list);
#define lf_2 t_list_node *__current;
#define lf_3 t_list_node *__next = __list->head;
#define lf_4 while ((__current = __next) &&
#define lf_5 (((__next && (__next = __next->next))) || 1) &&
#define lf_6(item) (((item) = __current->data) || 1))

#define lforeach(list, item) lf_1(list) lf_2 lf_3 lf_4 lf_5 lf_6(item)

#define lforeach_break (__next = NULL)

#define list_insert_before(i) (_list_add_node(__list, __current->prev, (i)))
#define list_insert_after(i) (_list_add_node(__list, __current, (i)))
#define list_insert(item) (list_insert_after(item))

#define list_remove(item) (_list_pop_node(__list, __current))
