...
static int logger_write(int level, char *msgl, char *message, va_list *varg)
{
  char buffer1[1024];
  char buffer2[1024];
  ...
  if (logger_is_init() && g_logger->level & level)
    {
      snprintf(buffer1, 1024, "{%s} %s", msgl, message);
...
int logger_message(char *message, ...)
{
  ...
  v = logger_write(LG_MESSAGE, "message", message, &varg);
  va_end(varg);
  return v;
}
...
int logger_warning(char *message, ...)
{
  ...
  va_start(varg, message);
  v = logger_write(LG_WARNING, "warning", message, &varg);
  va_end(varg);
  return v;
}
...
int logger_error(char *message, ...)
{
  ...
  va_start(varg, message);
  v = logger_write(LG_ERROR, "error", message, &varg);
  va_end(varg);
  return v;
}
...
int logger_debug(char *message, ...)
{
  ...
  va_start(varg, message);
  v = logger_write(LG_DEBUG, "debug", message, &varg);
  va_end(varg);
  return v;
}
